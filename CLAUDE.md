...
**Keep `.release-please-manifest.json` in its expanded shape** — `{`, newline, one indented entry, `}`. release-please *preserves the file's existing layout*, so a one-line manifest makes it write `{".":"0.1.0"}` with no spaces, and `oxfmt` then wants `{ ".": "0.1.0" }` — which turns `Lint & Format` red on every release PR, on a file nobody edited. Expanded, both tools agree and the release PR stays green. `envprism` has it this way.
